...
test("loadConfig throws aggregated errors for missing required values", () => {
    ...
    assert.throws(() => loadConfig(input), (error) => {
        ...
        assert.match(validationError.message, /HUNT_MONGODB_URI is required/);
        assert.match(validationError.message, /HUNT_JWT_SECRET is required/);
        assert.match(validationError.message, /HUNT_SIGNING_SECRET is required/);
        return true;
    });
});
test("loadConfig throws actionable error for invalid value types", () => {
    ...
    assert.throws(() => loadConfig(input), (error) => {
        ...
        const validationError = error;
        assert.match(validationError.message, /HUNT_PORT must be a positive integer/);
        assert.match(validationError.message, /HUNT_LOG_LEVEL has invalid value/);
        assert.match(validationError.message, /HUNT_MONGODB_URI must start with/);
        return true;
    });
});
