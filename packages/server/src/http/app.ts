...
import type { LogLevel, Logger, Middleware, RequestContext } from "./types.js";
import type {
...
function routeMiddleware(router: Router): Middleware {
  return async (ctx) => {
...
export function createHttpApp(options: HttpAppOptions): HttpApp {
  ...
  let authService: OAuthService | null = null;
  if (options.config.auth && options.authDependencies) {
    ...
    const jwtSigner = new HmacJwtSigner(
      options.config.auth.jwtSecret,
      options.config.auth.jwtExpiresInSeconds ?? 60 * 60 * 24,
    );
    const githubOAuthClient =
      options.authDependencies.githubOAuthClient ??
    ...
    const ingestionService = new IngestionService(
      {
        ...
        signatureMaxAgeSeconds:
          options.config.ingestion.signatureMaxAgeSeconds ?? 300,
        maxRequestsPerMinute:
          options.config.ingestion.maxRequestsPerMinute ?? 120,
      },
  ...
  const middlewareStack: Middleware[] = [
    ...
    createLoggingMiddleware(),
    createJsonBodyParserMiddleware(
      options.config.http?.maxBodyBytes !== undefined
