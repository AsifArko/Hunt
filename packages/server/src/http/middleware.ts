...
import type { Middleware, Next, RequestContext } from "./types.js";
...
export function composeMiddleware(middlewares: Middleware[]): Middleware {
  return async (ctx: RequestContext, next: Next) => {
    ...
    async function invoke(currentIndex: number): Promise<void> {
      if (currentIndex <= index) {
        throw new Error("next() called multiple times in middleware chain.");
      }
      index = currentIndex;

      const middlewareCandidate =
        currentIndex < middlewares.length ? middlewares[currentIndex] : next;
