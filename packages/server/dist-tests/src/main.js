...
async function main() {
    ...
    const app = createHttpApp({
        config: {
            ...
            ingestion: {
                ...
                signatureMaxAgeSeconds: Number(process.env.HUNT_SIGNATURE_MAX_AGE_SECONDS ?? "300"),
                maxRequestsPerMinute: Number(process.env.HUNT_INGESTION_MAX_REQUESTS_PER_MINUTE ?? "120"),
            },
            http: {
    ...
    if (!githubToken) {
        console.log("[hunt] metrics poller disabled (set HUNT_GITHUB_TOKEN to enable GitHub traffic sync).");
    }
}
